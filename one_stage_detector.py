...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        # Iterate over the FPN levels (p3, p4, p5)
        for level_name, feature_shape in dummy_out_shapes:
            in_channels = feature_shape[1]

            # Create lateral 1x1 conv layer
            lateral_layer = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)
            self.fpn_params[f"lateral_{level_name}"] = lateral_layer

            # Create output 3x3 conv layer
            output_layer = nn.Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
            self.fpn_params[f"output_{level_name}"] = output_layer
    ...
    def forward(self, images: torch.Tensor):
        ...
        for i in range(3):
            lateral_conv = self.fpn_params[f"lateral_c{i+3}"]
            output_conv = self.fpn_params[f"output_c{i+3}"]

            # Lateral connection
            lateral_feature = lateral_conv(backbone_feats[f'c{i+3}'])

            # Top-down connection
            if i > 0:
                prev_key = f"p{i+2}"
                lateral_feature = lateral_feature + F.interpolate(fpn_feats[prev_key], size =lateral_feature.shape[2:], mode='nearest')

            # Output feature
            fpn_feats[f"p{i+3}"] = output_conv(lateral_feature)

        return fpn_feats

        # for level_name in ["c3", "c4", "c5"]:
        #     lateral_layer = self.fpn_params[f"lateral_{level_name}"]
        #     output_layer = self.fpn_params[f"output_{level_name}"]

        #     backbone_feature = backbone_feats[level_name]

        #     # Create lateral feature
        #     lateral_feature = lateral_layer(backbone_feat[level_name])
        #     lateral_feature = lateral_feature.to(backbone_feature.device)
        #     adjusted_lateral_feature = nn.Conv2d(lateral_feature.shape[1], backbone_feature.shape[1], 
        #                                          kernel_size=1).to(backbone_feature.device)(lateral_feature)


        #     # Upsample lateral feature
        #     upsampled_lateral_feature = F.interpolate(adjusted_lateral_feature,
        #                                               size=backbone_feature.shape[2:],
        #                                               mode='nearest')

        #     fpn_feats[level_name] = upsampled_lateral_feature + backbone_feature

        #     # Apply output 3x3 conv layer
        #     fpn_feats[level_name] = output_layer(fpn_feats[level_name])
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level, feats in feats_per_fpn_level.items():
            # Apply the stem layers
            cls_feats = self.stem_cls(feats)
            box_feats = self.stem_box(feats)

            # Obtain predictions using the defined layers
            class_logits[level] = self.pred_cls(cls_feats).permute(0, 2, 3, 1).reshape(feats.size(0), -1,self.pred_cls.out_channels)
            boxreg_deltas[level] = self.pred_box(box_feats).permute(0, 2, 3,1).reshape(feats.size(0), -1, self.pred_box.out_channels)
            centerness_logits[level] = self.pred_ctr(box_feats).permute(0, 2,3,1).reshape(feats.size(0), -1,self.pred_ctr.out_channels)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for i in range(images.shape[0]):
            l_dict = {}
            l_dict["p3"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p3"], 
                                                          matched_gt_boxes[i]["p3"], self.backbone.fpn_strides["p3"])
            l_dict["p4"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p4"], 
                                                          matched_gt_boxes[i]["p4"], self.backbone.fpn_strides["p4"])
            l_dict["p5"] = fcos_get_deltas_from_locations(locations_per_fpn_level["p5"], 
                                                          matched_gt_boxes[i]["p5"], self.backbone.fpn_strides["p5"])
            matched_gt_deltas.append(l_dict)
        ...
        cls_target_logits = matched_gt_boxes[:,:,-1].to(torch.int64)
        # cls_logits = torch.cat((torch.zeros(1,self.num_classes),torch.eye(self.num_classes)),dim = 0).to(device=cls_target_logits.device)
        # gt_cls_vec = cls_logits[cls_target_logits]
        # loss_cls = sigmoid_focal_loss(pred_cls_logits,gt_cls_vec)
        cls_target_logits = F.one_hot(cls_target_logits+1, num_classes=self.num_classes+1).float()
        cls_target_logits = cls_target_logits[:, :, 1:] # Removing the background class
        loss_cls = sigmoid_focal_loss(pred_cls_logits, cls_target_logits, reduction="none", gamma = 1.42, alpha = 0.2)
        
        # box regression loss
        pred_boxreg_deltas = pred_boxreg_deltas.to(matched_gt_deltas.device)
        loss_box = F.l1_loss(pred_boxreg_deltas, matched_gt_deltas, reduction="none")*0.25
        loss_box[matched_gt_deltas < 0] *= 0.0
        
        # centerness loss
        
        ctr_target_logits = fcos_make_centerness_targets(matched_gt_deltas.view(-1,4))
        pred_ctr_logits = pred_ctr_logits.to(ctr_target_logits.device)
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.flatten(),ctr_target_logits,reduction = "none")
        loss_ctr[ctr_target_logits < 0] *= 0.0
        # pred_ctr_logits = torch.squeeze(pred_ctr_logits.reshape(-1, 1))
        # ctr_target_logits = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4))
        # pred_ctr_logits = pred_ctr_logits.to(ctr_target_logits.device)
        # loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits, ctr_target_logits, reduction="none")
        # loss_ctr[ctr_target_logits < 0] *= 0.0
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 1:
            # Replace "pass" statement with your code
            # level_cls_prob = level_cls_logits.sigmoid()
            # level_ctr_prob = level_ctr_logits.sigmoid()
            # level_pred_scores = torch.sqrt(level_cls_prob * level_ctr_prob)
            max_Scores, max_Classes = level_pred_scores.max(dim=1)
            
            
            # Step 2:
            # Replace "pass" statement with your code
            selected_indices = max_Scores > test_score_thresh
            
            level_pred_scores = max_Scores[selected_indices]
            level_pred_classes = max_Classes[selected_indices]



            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas[selected_indices], level_locations[selected_indices],
                                                              self.backbone.fpn_strides[level_name])
            # fcos_apply_deltas_to_locations()

            # level_pred_boxes[:, 0] = level_locations[:, 0] - level_deltas[:, 0]
            # level_pred_boxes[:, 1] = level_locations[:, 1] - level_deltas[:, 1]
            # level_pred_boxes[:, 2] = level_locations[:, 0] + level_deltas[:, 2]
            # level_pred_boxes[:, 3] = level_locations[:, 1] + level_deltas[:, 3]
            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            image_height, image_width = images.shape[-2:]
            level_pred_boxes[:, [0, 2]] = level_pred_boxes[:, [0, 2]].clamp(min=0, max=image_width - 1)
            level_pred_boxes[:, [1, 3]] = level_pred_boxes[:, [1, 3]].clamp(min=0, max=image_height - 1)
        ...
        # print("Pred_boxes", pred_boxes_all_levels.shape)
        # print("Pred_classes",pred_classes_all_levels.shape)
        # print("Pred_scores",pred_scores_all_levels.shape)
