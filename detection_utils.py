...
def fcos_get_deltas_from_locations(
    ...
    # Compute deltas (left, top, right, bottom) from the locations to GT box edges
    deltas_x = (x - gt_xmin) / stride
    deltas_y = (y - gt_ymin) / stride
    deltas_w = torch.abs((gt_xmax - x) / stride)
    deltas_h = torch.abs((gt_ymax - y) / stride)

    # Stack the deltas and return as a single tensor
    deltas = torch.stack((deltas_x, deltas_y, deltas_w, deltas_h), dim=1)
    

    is_background = (gt_boxes == -1).all(dim=1)
    background_deltas = torch.tensor([-1., -1., -1., -1.], dtype=gt_boxes.dtype)
    deltas = deltas.to(background_deltas.device)
    deltas[is_background] = background_deltas
...
def fcos_apply_deltas_to_locations(
    ...
    x1 = locations[:, 0] - unnormalized_deltas[:, 0] 
    y1 = locations[:, 1] - unnormalized_deltas[:, 1]  
    x2 = locations[:, 0] + unnormalized_deltas[:, 2]  
    y2 = locations[:, 1] + unnormalized_deltas[:, 3] 
    ...
    # Use torch.clamp to set negative values to zero
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # Compute centerness using the given formula
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        h, w = feat_shape[-2:]
        x_coords = torch.arange(0, w, device=device, dtype=dtype) + 0.5
        y_coords = torch.arange(0, h, device=device, dtype=dtype) + 0.5

        x_coords = x_coords * level_stride
        y_coords = y_coords * level_stride

        x_grid, y_grid = torch.meshgrid(x_coords, y_coords)
        locations = torch.stack([x_grid, y_grid], dim=-1).view(-1, 2)
